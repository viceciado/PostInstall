function Get-AvailableItems {
    ...
    try {
        ...
        if (-not $JsonPath) {
            # $PSScriptRoot aponta para Core/UI/; subir dois níveis para chegar na raiz do projeto.
            $projectRoot = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent

            if ($ItemType -eq "Programs") {
                $JsonPath = Join-Path $projectRoot "Data\AvailablePrograms.json"
            } else {
                $JsonPath = Join-Path $projectRoot "Data\AvailableTweaks.json"
            }
        }
