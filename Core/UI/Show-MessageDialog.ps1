function Show-MessageDialog {
    ...
    if ([string]::IsNullOrWhiteSpace($Message)) { throw "Parâmetro Message não pode ser vazio." }
    if ([string]::IsNullOrWhiteSpace($Title)) { throw "Parâmetro Title não pode ser vazio." }
        ...
        if ($iconText) {
            switch ($MessageType) {
                ...
                "Warning" {
                    ...
                    $iconText.Foreground = $global:PSConst.Colors.Warning
                }
                "Error" {
                    $iconText.Text = $global:PSConst.Icons.Error
                    $iconText.Foreground = $global:PSConst.Colors.Error
                }
                "Question" {
